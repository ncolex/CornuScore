...
const NewReviewPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!personIdentifier || !country || !phoneNumber || !category || !text) {
    ...
    const pseudoAuthor = `user***${phoneNumber.slice(-4)}`;
    const score = CATEGORIES[category].score;

    const success = await submitReview({ personIdentifier, country, phoneNumber, category, text, score, pseudoAuthor, evidenceUrl });
    setIsLoading(false);
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label htmlFor="phone" className="block text-sm font-medium text-gray-700 mb-1">
              Tu Teléfono (verificado, no público) <span className="text-red-500">*</span>
            </label>
            <input
              id="phone"
              type="tel"
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-pink-500 focus:border-pink-500 bg-gray-100"
              required
              readOnly
            />
          </div>
